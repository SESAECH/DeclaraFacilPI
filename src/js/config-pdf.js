...
function llenarPDF(){
    ...
    if(jsonResult.captura.formato =="SIMPLIFICADA"){
        ...
        tblCV();
        tblIngresos();
        if (jsonResult.captura.tipo_declaracion !="MODIFICACION"){tblDesempenoServidorPublico();}        
    }
    else{
        tblDatosGenerales();
        tblDomicilio();
...
function tblAdeudos(){
    let html="";
    html +='<tr><td colspan="3" style="background-color: #621132; color: #fff; font-size:14px;">14. ADEUDOS/PASIVOS</td></tr>';
    if(!jsonResult.declaracion.situacionPatrimonial.adeudos.ninguno){
        Object.keys(jsonResult.declaracion.situacionPatrimonial.adeudos.adeudo).forEach(function (index) {
            ...
            html+=" <td>SALDO INSOLUTO (SITUACIÓN ACTUAL)</td>";
            ...
            html+=" <td style='text-align: right;'>" + format(nodo.saldoInsolutoSituacionActual.valor) + nodo.saldoInsolutoSituacionActual.moneda + "</td>";
...
function tblPrestamoOComodato(){
    let html="";
    html +='<tr><td colspan="3" style="background-color: #621132; color: #fff; font-size:14px;">15. PRÉSTAMO O COMODATO POR TERCEROS</td></tr>';
