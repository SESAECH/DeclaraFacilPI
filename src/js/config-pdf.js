...
window.generarPDF = function generarPDF(){
    ...
    doc.text(moment().format("L LTS A"), 50, 40);
   ...
   /*  doc.text('TIPO DE DECLARACIÓN: ' + captura.tipo_declaracion, 195, 30, {maxWidth: 195, align: "right"})
    ...
    //finalY = doc.lastAutoTable.finalY;
    //doc.autoTable({ html: '#pdfTitulo4', margin: { top: 25, bottom:0 }, styles: { fontSize: 12 }, startY: finalY + 10, useCss: true, pageBreak: 'always' });  
    finalY = doc.lastAutoTable.finalY;
    ...
    //EXPERIENCIA LABORAL.
    //finalY = doc.lastAutoTable.finalY;
    //doc.autoTable({ html: '#pdfTitulo5', margin: { top: 25, bottom:0 }, styles: { fontSize: 12 }, startY: finalY + 10, useCss: true, pageBreak: 'avoid' });
    finalY = doc.lastAutoTable.finalY;
...
function tblIngresos(){
    let nodoIngresos = jsonResult.declaracion.situacionPatrimonial.ingresos
    let html="";
    html +='<tr><td colspan="2" style="background-color: #621132; color: #fff; font-size:14px;">6. INGRESOS NETOS DEL DECLARANTE, PAREJA Y/O DEPENDIENTES ECONÓMICOS</td></tr>';

    html +='<tr>\
                <td style="background-color: #dee2e6;">I.- REMUNERACIÓN MENSUAL NETA DEL DECLARANTE POR SU CARGO PÚBLICO (POR CONCEPTO DE SUELDOS, HONORARIOS, COMPENSACIONES, BONOS Y OTRAS PRESTACIONES)(CANTIDADES NETAS DESPUÉS DE IMPUESTOS)</td>\
                <td style="text-align: right;">' + format(nodoIngresos.remuneracionMensualCargoPublico.valor) + " " + nodoIngresos.remuneracionMensualCargoPublico.moneda +'</td>\
            </tr>\
            <tr>\
                <td style="background-color: #dee2e6;">II.- OTROS INGRESOS MENSUALES DEL DECLARANTE (SUMA DEL II.1 AL II.4)</td>\
                <td style="text-align: right;">' + format(nodoIngresos.otrosIngresosMensualesTotal.valor) + " " + nodoIngresos.otrosIngresosMensualesTotal.moneda + '</td>\
            </tr>\
            <tr style="background-color: #dee2e6;">\
                <td style="">II.1.- POR ACTIVIDAD INDUSTRIAL, COMERCIAL Y/O EMPRESARIAL (DESPUÉS DE IMPUESTOS)</td>\
                <td style="text-align: right;">' + format(nodoIngresos.actividadIndustrialComercialEmpresarial.remuneracionTotal.valor) + " " + nodoIngresos.actividadIndustrialComercialEmpresarial.remuneracionTotal.moneda + '</td>\
            </tr>';

    Object.keys(nodoIngresos.actividadIndustrialComercialEmpresarial.actividades).forEach(function (index) {
        let nodo = nodoIngresos.actividadIndustrialComercialEmpresarial.actividades[index];
        html +='<tr>\
    ...
    html +='<tr style="background-color: #dee2e6;">\
                <td style="">II.4.- OTROS INGRESOS NO CONSIDERADOS A LOS ANTERIORES (DESPUÉS DE IMPUESTOS)</td>\
    ...
    html +='<tr>\
                ...
                <td style="text-align: right;">' + format(nodoIngresos.ingresoMensualNetoDeclarante.valor) + " " + nodoIngresos.ingresoMensualNetoDeclarante.moneda +'</td>\
            </tr>\
            <tr>\
                <td style="background-color: #dee2e6;">B.- INGRESO MENSUAL DE LA PAREJA Y/O DEPENDIENTES ECONÓMICOS (DESPUÉS DE IMPUESTOS)</td>\
                <td style="text-align: right;">' + format(nodoIngresos.ingresoMensualNetoParejaDependiente.valor) + " " + nodoIngresos.ingresoMensualNetoParejaDependiente.moneda + '</td>\
            </tr>\
            <tr>\
                ...
                <td style="text-align: right;">' + format(nodoIngresos.totalIngresosMensualesNetos.valor) + " " + nodoIngresos.totalIngresosMensualesNetos.moneda + '</td>\
            </tr>';   
...
function tblDesempenoServidorPublico(){
    let nodo = jsonResult.declaracion.situacionPatrimonial.actividadAnualAnterior
    let html="";
    html +='<tr><td colspan="2" style="background-color: #621132; color: #fff; font-size:14px;">7.¿TE DESEMPEÑASTE COMO SERVIDOR PÚBLICO EL AÑO INMEDIATO ANTERIOR?</td></tr>';
    html +='<tr>\
